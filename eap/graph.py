...
class Graph:
    ...
    def apply_topn(self, n:int, reset=True, absolute: bool = True):
        ...
        Args:
            ...
            reset (bool): whether to reset the in_graph attribute before applying the filter
            absolute (bool): whether to take the absolute value of the scores before applying the threshold"""
    ...
    def apply_modifications(self, nodes_to_remove: Optional[List[str]] = None, edges_to_remove: Optional[List[str]] = None):
        """
        Modifies the graph by removing specified nodes and edges (setting in_graph=False).
        Also prunes dead nodes subsequently.

        Args:
            nodes_to_remove (Optional[List[str]]): List of node names to remove.
            edges_to_remove (Optional[List[str]]): List of edge names to remove.
        """
        ...
        if nodes_to_remove:
            for node_name in nodes_to_remove:
                if node_name in self.nodes:
                    if self.nodes[node_name].in_graph:
                         self.nodes[node_name].in_graph = False
                         nodes_removed_count += 1
                         # Also mark associated edges as not in graph initially
                         for edge in self.nodes[node_name].parent_edges:
                             if edge.in_graph:
                                 edge.in_graph = False
                                 # Don't double count if edge is also in edges_to_remove
                                 if not edges_to_remove or edge.name not in edges_to_remove:
                                     edges_removed_count += 1
                         for edge in self.nodes[node_name].child_edges:
                              if edge.in_graph:
                                 edge.in_graph = False
                                 if not edges_to_remove or edge.name not in edges_to_remove:
                                     edges_removed_count += 1
                else:
                    print(f"Warning: Node '{node_name}' not found in graph during modification.")

        if edges_to_remove:
            for edge_name in edges_to_remove:
                if edge_name in self.edges:
                    # Only count if it wasn't already removed by node removal
                    if self.edges[edge_name].in_graph:
                        self.edges[edge_name].in_graph = False
                        edges_removed_count += 1
                else:
                    print(f"Warning: Edge '{edge_name}' not found in graph during modification.")

        # Prune again after modifications to ensure consistency
        print(f"Applying modifications: removing {nodes_removed_count} specified nodes and {edges_removed_count} specified edges (before pruning)." )
        self.prune_dead_nodes()
        print(f"Graph after modifications and pruning: {self.count_included_nodes()} nodes, {self.count_included_edges()} edges.")

    def remove_specific_nodes(self, nodes_to_remove: List[str]):
        """
        从电路中删除指定的节点及其相关的所有边。
        这个方法专门用于在EAP分析完成后，从已修剪的电路中进一步删除特定节点。
        
        Args:
            nodes_to_remove (List[str]): 要删除的节点名称列表 (例如: ["a10.h2", "m5"])
        """
        ...
        print(f"开始从电路中删除指定节点: {nodes_to_remove}")
        ...
        for node_name in nodes_to_remove:
            if node_name in self.nodes:
                ...
                if node.in_graph:
                    ...
                    print(f"  节点 {node_name} 不在当前电路中，跳过")
            else:
                print(f"  警告: 节点 '{node_name}' 在图中不存在")
        
        # 删除节点后重新修剪死节点，确保电路的完整性
        if nodes_removed_count > 0:
            print(f"删除了 {nodes_removed_count} 个节点和 {edges_removed_count} 条相关边")
            print("重新修剪死节点以确保电路完整性...")
            self.prune_dead_nodes()
            final_nodes = self.count_included_nodes()
            final_edges = self.count_included_edges()
            print(f"最终电路: {final_nodes} 个节点, {final_edges} 条边")
        else:
            print("没有删除任何节点")
