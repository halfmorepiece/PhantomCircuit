...
def get_circuit_logits(model: HookedTransformer, graph: Graph, data: tuple, prune:bool=True, quiet=False):
    ...
    Args:
        ...
        data: tuple (clean, corrupted, label) where clean and corrupted can be either strings or tensors
        prune: bool, if True, prune the graph before evaluation.
        quiet: bool, if True, do not display progress bars.
    Returns:
        Tensor, the logits from the circuit.
    """
    if prune:
    ...
    if empty_circuit:
        # print("Warning: empty circuit") # Keep warnings maybe?
        pass
    ...
    in_graph_matrix = torch.zeros((graph.n_forward, graph.n_backward), device=model_device, dtype=model.cfg.dtype)
    ...
    # print(f"get_circuit_logits - 数据类型: clean={type(clean)}, corrupted={type(corrupted)}") # Removed
    
    # 处理不同类型的输入
    clean_tokens, attention_mask, input_lengths, n_pos = tokenize_plus(model, clean)
    corrupted_tokens, _, _, _ = tokenize_plus(model, corrupted)
    
    batch_size = 1 if clean_tokens.dim() == 1 else len(clean_tokens)
...
def evaluate_graph(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metrics: Union[List[Callable[[Tensor], Tensor]],Callable[[Tensor], Tensor]], prune:bool=True, quiet=False):
    ...
    for clean, corrupted, label in dataloader:
        # print(f"evaluate_graph - 数据类型: clean={type(clean)}, corrupted={type(corrupted)}") # Removed
...
def evaluate_baseline(model: HookedTransformer, dataloader:DataLoader, metrics: Union[List[Callable[[Tensor], Tensor]],Callable[[Tensor], Tensor]], run_corrupted=False, quiet=False):
    ...
    dataloader = dataloader if quiet else tqdm(dataloader)
    for clean, corrupted, label in dataloader:
        # print(f"evaluate_baseline - 数据类型: clean={type(clean)}, corrupted={type(corrupted)}") # Removed
