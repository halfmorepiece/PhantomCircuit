...
from data_utils import align_subject_tokens
...
def make_hooks_and_matrices(model: HookedTransformer, graph: Graph, batch_size:int , n_pos:int, scores: torch.Tensor, detach=True):
    ...
    if scores is not None and scores.device != model_device:
        print(f"Warning: Scores tensor is on device {scores.device}, expected {model_device}. Using scores device.")
        activation_difference_device = scores.device
    else:
        activation_difference_device = model_device
        
    activation_difference = torch.zeros((batch_size, n_pos, graph.n_forward, model.cfg.d_model), device=activation_difference_device, dtype=model.cfg.dtype)
    ...
    def gradient_hook(prev_index: Union[slice, int], bwd_index: Union[slice, int], gradients:torch.Tensor, hook):
        ...
        try:
            ...
            s = einsum(activation_difference[:, :, :prev_index].to(grads.device), grads,'batch pos forward hidden, batch pos backward hidden -> forward backward')
...
def tokenize_plus(model: HookedTransformer, inputs: Union[List[str], str, torch.Tensor]):
    """
    Tokenizes the input strings or handles already tokenized inputs using the provided model.

    Args:
        model (HookedTransformer): The model used for tokenization.
        inputs (Union[List[str], str, torch.Tensor]): The input strings to be tokenized or already tokenized tensor.
    ...
    if isinstance(inputs, torch.Tensor):
        # 确保batch维度存在
        if inputs.dim() == 1:
            tokens = inputs.unsqueeze(0)
        else:
            tokens = inputs
            
        # 创建attention mask (所有位置都是1)
...
def get_scores_eap(model: HookedTransformer, graph: Graph, dataloader:DataLoader, metric: Callable[[Tensor], Tensor], quiet=False):
    # 获取模型设备
    model_device = next(model.parameters()).device
    scores = torch.zeros((graph.n_forward, graph.n_backward), device=model_device, dtype=model.cfg.dtype)    
...
def get_scores_eap_ig_case(model: HookedTransformer, graph: Graph, data: tuple, metric: Callable[[Tensor], Tensor], steps=30, quiet=False):
    ...
    if isinstance(clean, str) and isinstance(corrupted, str):
        clean_aligned, corrupted_aligned, _ = align_subject_tokens(model, clean, corrupted)
        clean_tokens = clean_aligned.unsqueeze(0)
        corrupted_tokens = corrupted_aligned.unsqueeze(0)
    elif isinstance(clean, torch.Tensor) and isinstance(corrupted, torch.Tensor):
        if clean.dim() == 1:
            clean_tokens = clean.unsqueeze(0)
        else:
            clean_tokens = clean
            
        if corrupted.dim() == 1:
            corrupted_tokens = corrupted.unsqueeze(0)
        else:
            corrupted_tokens = corrupted
            
        if clean_tokens.shape[1] != corrupted_tokens.shape[1]:
            raise ValueError(f"序列长度不匹配，无法继续分析")
    else:
        raise TypeError(f"不支持的输入类型")
    
    # 使用tokenize_plus处理已对齐的tokens
    attention_mask = get_attention_mask(model.tokenizer, clean_tokens, True)
    input_lengths = attention_mask.sum(1)
    n_pos = attention_mask.size(1)

    (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks), activation_difference = make_hooks_and_matrices(model, graph, clean_tokens.shape[0], n_pos, scores)
...
def attribute(model: HookedTransformer, graph: Graph, dataloader: Union[DataLoader, tuple], metric: Callable[[Tensor], Tensor], aggregation='sum', method: Union[Literal['EAP', 'EAP-IG', 'EAP-IG-partial-activations', 'EAP-IG-activations', 'clean-corrupted', 'EAP-IG-case']]='EAP-IG', ig_steps: Optional[int]=5, quiet=False):
    ...
    # 确保转换为float32，解决BFloat16不支持问题
    scores = scores.to(torch.float32).cpu().numpy()

    for edge in tqdm(graph.edges.values(), total=len(graph.edges), disable=quiet):
